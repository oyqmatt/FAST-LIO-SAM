...
void FAST_LIO_SAM_CLASS::odom_pcd_cb(
    const nav_msgs::msg::Odometry::ConstSharedPtr &odom_msg,
    const sensor_msgs::msg::PointCloud2::ConstSharedPtr &pcd_msg) {
  Eigen::Matrix4d last_odom_tf_;
  last_odom_tf_ = m_current_frame.pose_eig;  // to calculate delta
  m_current_frame =
      pose_pcd(*odom_msg, *pcd_msg,
               m_current_keyframe_idx);  // to be checked if keyframe or not
  ...
  if (!m_init)  //// init only once
  {
    ...
    m_corrected_current_pcd_pub->publish(
        pcl_to_pcl_ros(m_current_frame.pcd, m_map_frame));
    // graph
    gtsam::noiseModel::Diagonal::shared_ptr prior_noise_ =
        gtsam::noiseModel::Diagonal::Variances(
            (gtsam::Vector(6) << 1e-4, 1e-4, 1e-4, 1e-4, 1e-4, 1e-4)
                .finished());  // rad*rad, meter*meter
    m_gtsam_graph.add(gtsam::PriorFactor<gtsam::Pose3>(
        0, pose_eig_to_gtsam_pose(m_current_frame.pose_eig), prior_noise_));
    m_init_esti.insert(m_current_keyframe_idx,
                       pose_eig_to_gtsam_pose(m_current_frame.pose_eig));
    {
      lock_guard<mutex> lock(m_realtime_pose_mutex);
      m_odom_delta =
          m_odom_delta * last_odom_tf_.inverse() * m_current_frame.pose_eig;
      m_current_frame.pose_corrected_eig = m_last_corrected_pose * m_odom_delta;
      geometry_msgs::msg::PoseStamped current_pose_stamped_ =
          pose_eig_to_pose_stamped(m_current_frame.pose_corrected_eig,
                                   m_map_frame);
      m_realtime_pose_pub->publish(current_pose_stamped_);
      tf2::Transform transform_;
      transform_.setOrigin(tf2::Vector3(current_pose_stamped_.pose.position.x,
                                        current_pose_stamped_.pose.position.y,
                                        current_pose_stamped_.pose.position.z));
      transform_.setRotation(
          tf2::Quaternion(current_pose_stamped_.pose.orientation.x,
      ...
      transform_stamped_.header.frame_id = m_map_frame;
      transform_stamped_.child_frame_id = "robot";
      ...
      transform_stamped_.transform.rotation.w = transform_.getRotation().getW();
      transform_stamped_.transform.rotation.x =
          transform_.getRotation().getAxis().getX();
      transform_stamped_.transform.rotation.y =
          transform_.getRotation().getAxis().getY();
      transform_stamped_.transform.rotation.z =
          transform_.getRotation().getAxis().getZ();

      m_broadcaster_->sendTransform(transform_stamped_);
    }
    ...
    m_corrected_current_pcd_pub->publish(pcl_to_pcl_ros(
        tf_pcd(m_current_frame.pcd, m_current_frame.pose_corrected_eig),
        m_map_frame));
    ...
    if (check_if_keyframe(m_current_frame, m_keyframes.back())) {
      // 2-2. if so, save
      {
        ...
        m_keyframes.push_back(m_current_frame);
        m_not_processed_keyframe =
            m_current_frame;  // to check loop in another thread
      }
      ...
      gtsam::noiseModel::Diagonal::shared_ptr odom_noise_ =
          gtsam::noiseModel::Diagonal::Variances(
              (gtsam::Vector(6) << 1e-4, 1e-4, 1e-4, 1e-2, 1e-2, 1e-2)
                  .finished());
      gtsam::Pose3 pose_from_ = pose_eig_to_gtsam_pose(
          m_keyframes[m_current_keyframe_idx - 1].pose_corrected_eig);
      gtsam::Pose3 pose_to_ =
          pose_eig_to_gtsam_pose(m_current_frame.pose_corrected_eig);
      {
        lock_guard<mutex> lock(m_graph_mutex);
        m_gtsam_graph.add(gtsam::BetweenFactor<gtsam::Pose3>(
            m_current_keyframe_idx - 1, m_current_keyframe_idx,
            pose_from_.between(pose_to_), odom_noise_));
      ...
      // m_init_esti).optimize(); // cf. isam.update vs values.LM.optimize
      {
        lock_guard<mutex> lock(m_graph_mutex);
      ...
      high_resolution_clock::time_point t5_ = high_resolution_clock::now();
      {
        ...
        m_last_corrected_pose = gtsam_pose_to_pose_eig(
            m_corrected_esti.at<gtsam::Pose3>(m_corrected_esti.size() - 1));
      ...
      if (m_loop_added_flag) {
        ...
        for (size_t i = 0; i < m_corrected_esti.size(); ++i) {
          m_keyframes[i].pose_corrected_eig =
              gtsam_pose_to_pose_eig(m_corrected_esti.at<gtsam::Pose3>(i));
        }
      ...
      RCLCPP_INFO(this->get_logger(),
                  ...
                  duration_cast<microseconds>(t5_ - t4_).count() / 1e3,
                  duration_cast<microseconds>(t6_ - t5_).count() / 1e3,
                  duration_cast<microseconds>(t6_ - t1_).count() / 1e3);
    }
  }
  return;
}

void FAST_LIO_SAM_CLASS::on_timer_callback() {
  if (!m_init) return;
  ...
  // from not_proc_key_copy_ keyframe to old keyframes in threshold radius, get
  // the closest keyframe
  int closest_keyframe_idx_ =
      get_closest_keyframe_idx(not_proc_key_copy_, keyframes_copy_);
  if (closest_keyframe_idx_ >= 0)  // if exists
  {
    // ICP to check loop (from front_keyframe to closest keyframe's neighbor)
    icp_key_to_subkeys(not_proc_key_copy_, closest_keyframe_idx_,
                       keyframes_copy_);
    ...
    {
      Eigen::Matrix4d pose_between_eig_ =
          m_icp.getFinalTransformation().cast<double>();
      gtsam::Pose3 pose_from_ = pose_eig_to_gtsam_pose(
          pose_between_eig_ * not_proc_key_copy_.pose_corrected_eig);
      gtsam::Pose3 pose_to_ = pose_eig_to_gtsam_pose(
          keyframes_copy_[closest_keyframe_idx_].pose_corrected_eig);
      gtsam::noiseModel::Diagonal::shared_ptr loop_noise_ =
          gtsam::noiseModel::Diagonal::Variances((gtsam::Vector(6) << score_,
                                                     ...
                                                     .finished());
      {
        lock_guard<mutex> lock(m_graph_mutex);
        m_gtsam_graph.add(gtsam::BetweenFactor<gtsam::Pose3>(
            not_proc_key_copy_.idx, closest_keyframe_idx_,
            pose_from_.between(pose_to_), loop_noise_));
      }
      m_loop_idx_pairs.push_back(
          {not_proc_key_copy_.idx, closest_keyframe_idx_});  // for vis
  ...
  RCLCPP_INFO(this->get_logger(), "copy: %.1f, loop: %.1f",
              duration_cast<microseconds>(t2_ - t1_).count() / 1e3,
              duration_cast<microseconds>(t3_ - t2_).count() / 1e3);
...
void FAST_LIO_SAM_CLASS::on_vis_timer_callback() {
  if (!m_init) return;
  ...
  if (m_loop_added_flag_vis) {
    ...
    for (size_t i = 0; i < corrected_esti_copy_.size(); ++i) {
      ...
      corrected_odoms_.points.emplace_back(pose_.translation().x(),
                                           pose_.translation().y(),
                                           pose_.translation().z());
      corrected_path_.poses.push_back(
          gtsam_pose_to_pose_stamped(pose_, m_map_frame));
    }
  ...
  //// 2. publish odoms, paths
  {
    ...
    m_corrected_odom_pub->publish(
        pcl_to_pcl_ros(m_corrected_odoms, m_map_frame));
    m_corrected_path_pub->publish(m_corrected_path);
  }

  //// 3. global map
  if (m_global_map_vis_switch &&
      m_corrected_pcd_map_pub->get_subscription_count() >
          0)  // save time, only once
  {
    ...
    m_corrected_pcd_map_pub->publish(
        pcl_to_pcl_ros(corrected_map_, m_map_frame));
    m_global_map_vis_switch = false;
  }
  if (!m_global_map_vis_switch &&
      m_corrected_pcd_map_pub->get_subscription_count() == 0) {
  ...
  RCLCPP_INFO(this->get_logger(), "vis: %.1fms",
              duration_cast<microseconds>(tv2_ - tv1_).count() / 1e3);
  return;
}
